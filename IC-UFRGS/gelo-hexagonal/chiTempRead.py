...
pasta_chi = f"./hopping-{hop}/chiTempOutput"
...
for i in os.listdir(path):
    ...
    if os.path.isfile(os.path.join(path,i)) and f'chiTemp{pol}-' in i:
    
        files.append(i)
        wList.append(float(i.replace(f'chiTemp{pol}-','').replace('.txt', '')))
...
for i in range(len(wList)):
    ...
    filename = f'{pasta_chi}/chiTemp{pol}-{w}.txt'
