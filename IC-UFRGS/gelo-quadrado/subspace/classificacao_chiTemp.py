...
def wList_Read(N, hop):
    
    path = f'./{N}-hopping-{hop}/w_state/'
    wList = []
...
path = f"./{N}-hopping-{hop}/chiTempOutput/"

wList = wList_Read(N, hop)
...
for i in range(len(wList)):
    ...
    classe = Classifica(T[0:16], chiT[0:16])
    if classe=='anomala':
        print(f'w: {w}')
        
        plt.plot(T[0:16], chiT[0:16])
