...
Tlist = np.arange(0.5, 10, 0.05)

print('Leitura')

n, m, Pnm, w = np.loadtxt(f"{pasta}/{N}_Pxxnm.txt", unpack=True)

val, vec = read_autocoisas(f"{pasta}/{N}_autocoisas.txt")

print('Calculo Chi')

wList, chiList, stateList, fParticaoList = soma(w, Pnm, n, m, Tlist, val, vec)
