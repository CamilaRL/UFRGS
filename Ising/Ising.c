...
void main(void){
    ...
    do
    {
        ...
        // Time evolution
        for(int t = 0 ; t < MCS_max ; t++)
        {   
            //printf("\nt: %i E: %f M: %f\n", t, E, M);
            // Metropolis Method Aplication
            metropolis(s, &E, &M, T);
            
            if(t < MCS_max) // Após o equilíbrio
            {
        ...
        E_med = E_med/(MCS_max*L2);
        M_med = M_med/(MCS_max*L2);
        E2_med = E2_med/(MCS_max*L2);
        M2_med = M2_med/(MCS_max*L2);

        X_med = (M2_med - (M_med*M_med))/(L2*kB*T);
        C_med = (E2_med - (E_med*E_med))/(kB*T*T);
